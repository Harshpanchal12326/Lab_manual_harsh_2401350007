...
interface RecordActions {
    ...
    // These methods now declare that they might throw our custom exception
...
class Student extends Person {
    // Using Wrapper Classes (Integer, Double) for fields is optional, 
    // but we will use them for data conversion in Main.
    int rollNo;
    String course;
    double marks;
    char grade;
...
class StudentManager implements RecordActions {
    ...
    public void addStudent(Student s) {
        ...
        // MULTITHREADING: Simulate a loading/saving delay
        Thread loadingThread = new Thread(new LoadingTask());
        loadingThread.start();
        
        try {
            // We wait for the loading to finish before confirming success
            loadingThread.join();
        } catch (InterruptedException e) {
            e.printStackTrace();
        }
    ...
    public void viewAll() {
        if (studentList.isEmpty()) {
            ...
            for (Student s : studentList) s.displayInfo();
        }
    }
}
...
public class lab_assignment3 {
    public static void main(String[] args) {
        ...
        while (true) {
            ...
            try {
                // WRAPPER CLASSES: Using Integer.parseInt for robust conversion
                // This avoids the "skipping line" bug common with scanner.nextInt()
                ...
                // Handles cases where user types "abc" instead of a number
