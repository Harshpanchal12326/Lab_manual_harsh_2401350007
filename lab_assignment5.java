...
// Custom exception for when a student record is not found
...
// Interface defining the contract for our system
...
// 4. MULTITHREADING
// Simulates a loading process (e.g., saving to server/disk)
...
class StudentManager implements IStudentRecord {
    ...
    public StudentManager() {
        ...
        loadRecords(); // Load data automatically on startup
    }
    ...
    public void saveRecords() {
        ...
        t.start();
        try {
            t.join(); // Wait for animation to finish
    ...
    public void updateStudent(int rollNo, double newMarks) throws StudentNotFoundException {
        ...
        if (s == null) throw new StudentNotFoundException("Update Failed: Student not found.");
    ...
    public void deleteStudent(int rollNo) throws StudentNotFoundException {
        Student s = findStudentHelper(rollNo);
        if (s == null) throw new StudentNotFoundException("Delete Failed: Student not found.");
    ...
    public Student searchStudent(int rollNo) throws StudentNotFoundException {
        Student s = findStudentHelper(rollNo);
        if (s == null) throw new StudentNotFoundException("Search Failed: Student not found.");
        return s;
    }
